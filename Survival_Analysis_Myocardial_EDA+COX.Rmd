...
for (i in 1:(k)){
  ...
  like_indiv <- (exp(X %*% betas) / sum(exp(X %*% betas))) ** (TRACE$status == 9)
  log_likelihood_global[[i]] <- sum(log(like_indiv))
}

CVE <- -2 * sum(unlist(log_likelihood_global) - unlist(train_loglik))
